...
class EnhancedScanner:
    def __init__(self):
        ...
        try:
            ...
            self.logger.info("✅ Scanner on-chain initialisé (événements PairCreated)")
        except Exception as e:
    ...
    async def fetch_new_tokens(self) -> List[Dict]:
        """
        🔥 NOUVEAU: Récupère les tokens via scanner on-chain (événements PairCreated).
        Remplace l'approche DexScreener/GeckoTerminal par un scan direct des événements blockchain.
        """
        try:
            # Vérifier que le scanner on-chain est disponible
            if not hasattr(self, 'onchain_scanner') or not self.onchain_scanner:
                self.logger.error("⚠️  Scanner on-chain non disponible, fallback DB")
                return await self._fetch_tokens_from_db()

            # Scanner les tokens entre min_token_age_hours et max_token_age_hours
            self.logger.info(
                f"🔍 Scan on-chain: tokens entre {self.min_token_age_hours}h et {self.max_token_age_hours}h"
            )

            raw_tokens = self.onchain_scanner.scan_tokens_in_age_window(
                min_hours=self.min_token_age_hours,
                max_hours=self.max_token_age_hours,
                max_results=self.batch_size
            )

            if not raw_tokens:
                self.logger.info("Aucun token on-chain trouvé dans la fenêtre")
                return []

            # Enrichir avec métadonnées ERC20
            self.logger.info(f"📊 Enrichissement de {len(raw_tokens)} tokens...")
            enriched_tokens = []

            for token_data in raw_tokens:
                try:
                    metadata = self.onchain_scanner.get_token_metadata(
                    ...
                    enriched = {
                        ...
                        'factory': token_data['factory_name'],
                        'block_created': token_data['block_created'],
                        'age_hours': token_data['age_hours'],
                        'discovered_at': token_data.get('discovered_at', None)
                    }
                    ...
                    self.logger.warning(
                        f"⚠️  Erreur enrichissement {token_data['token_address'][:8]}...: {e}"
                    )
                    continue

            self.logger.info(
                f"✅ {len(enriched_tokens)}/{len(raw_tokens)} tokens on-chain enrichis"
            )
            return enriched_tokens

        except Exception as e:
            self.logger.error(f"❌ Erreur scan on-chain: {e}")
            # Fallback vers DB en cas d'erreur critique
            return await self._fetch_tokens_from_db()
